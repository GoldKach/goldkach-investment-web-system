...
function PasswordInput({ id, value, onChange, placeholder, disabled, required }: {
...
export function AdminSettingsForm({ user }: AdminSettingsFormProps) {
  ...
  const handleImageUpload = async (res: any) => {
    ...
    try {
      await updateUserById(user.id, { imageUrl: url });
      setImageUrl(url);
      setShowUpload(false);
  ...
  const handleRemoveImage = async () => {
    try {
      await updateUserById(user.id, { imageUrl: "" });
      setImageUrl("");
          ...
          <CardContent>
            <div className="flex items-center gap-6">
              <div className="relative shrink-0">
                <Avatar className="h-20 w-20 border-2 border-slate-200 dark:border-[#1e2d5a]">
                  <AvatarImage src={imageUrl} alt={displayName} />
                  <AvatarFallback className="bg-[#2B2F77] text-white text-xl font-bold">{initials}</AvatarFallback>
                </Avatar>
                {imageUrl && (
                  <button
                    type="button"
                ...
                {!showUpload ? (
                  ...
                  <div className="space-y-2">
                    <UploadDropzone
                      ...
                      className="border-2 border-dashed border-slate-200 dark:border-[#1e2d5a] rounded-lg"
                    />
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleProfileSave} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">Role (read-only)</Label>
                <Input value={user.role} disabled className="text-slate-400" />
              </div>
