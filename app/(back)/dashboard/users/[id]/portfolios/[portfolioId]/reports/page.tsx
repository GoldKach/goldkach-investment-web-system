...
export default async function AdminPortfolioReportsPage({ params }: Props) {
      ...
      {portfolioItem && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Gain / Loss",
              value: fmt(portfolioItem.totalLossGain),
              color: portfolioItem.totalLossGain >= 0 ? "text-green-600" : "text-red-500",
            },
            {
              label: "Return",
              value: `${portfolioItem.returnPct >= 0 ? "+" : ""}${portfolioItem.returnPct.toFixed(2)}%`,
              color: portfolioItem.returnPct >= 0 ? "text-green-600" : "text-red-500",
            },
          ].map((m) => (
            <Card key={m.label}>
              <CardContent className="pt-4 pb-3">
                <p className="text-xs text-slate-400">{m.label}</p>
                <p className={`text-lg font-bold mt-0.5 ${m.color ?? "text-slate-800 dark:text-white"}`}>
                  {m.value}
                </p>
              </CardContent>
            </Card>
...
function BackLink({ userId }: { userId: string }) {
  return (
    <Link
