...
export function UserPortfolioDetail({ userPortfolio }: PortfolioDetailProps) {
  ...
  const totalCost = userPortfolio.userAssets.reduce((sum: number, asset: any) => sum + asset.costPrice, 0)
  const totalValue = userPortfolio.userAssets.reduce((sum: number, asset: any) => sum + asset.closeValue, 0)
  const totalLossGain = userPortfolio.userAssets.reduce((sum: number, asset: any) => sum + asset.lossGain, 0)
  const lossGainPercentage = totalCost > 0 ? (totalLossGain / totalCost) * 100 : 0
  const isPositive = totalLossGain >= 0

  // ✅ FIXED: Use asset.asset instead of asset.portfolioAsset.asset
  const assets = userPortfolio.userAssets.map((asset: any) => {
    const assetLossGainPercentage = asset.costPrice > 0 ? (asset.lossGain / asset.costPrice) * 100 : 0

    return {
      id: asset.id,
      assetName: asset.asset?.description ?? "Unknown",
      assetSymbol: asset.asset?.symbol ?? "—",
      sector: asset.asset?.sector ?? "Unknown",
      cost: asset.costPrice,
      stock: asset.stock,
      closeValue: asset.closeValue,
      lossGain: asset.lossGain,
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <ChartContainer
                  config={{ value: { label: "Portfolio Value", color: "#3b82f6" } }}
                  className="h-[300px]"
                >
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={performanceData}>
        ...
        <TabsContent value="assets" className="space-y-6">
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {assets.map((asset: any, index: number) => {
                      ...
                      const allocationPercent = ((asset.cost / totalCost) * 100).toFixed(1)
                      const costPerShare = asset.stock > 0 ? asset.cost / asset.stock : 0
                      const closePrice = asset.stock > 0 ? asset.closeValue / asset.stock : 0
                      ...
                      return (
                        <tr key={asset.id} className="border-b hover:bg-muted/50 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <p className="text-sm font-medium truncate max-w-[200px]">{asset.assetName}</p>
                          </td>
                          <td className="py-3 px-4">
                            <span className="text-sm text-muted-foreground">{asset.sector}</span>
                          </td>
                          ...
                          <td className="py-3 px-4 text-right">
                            <span className="text-sm font-mono">${closePrice.toFixed(2)}</span>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <span className="text-sm font-semibold">${asset.closeValue.toLocaleString()}</span>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <div className="flex flex-col items-end gap-0.5">
                              <span className={`text-sm font-bold ${assetIsPositive ? "text-green-600" : "text-red-600"}`}>
                                ${Math.abs(asset.lossGain).toLocaleString()}
                              </span>
                              <span className={`text-xs font-semibold flex items-center gap-1 ${assetIsPositive ? "text-green-600" : "text-red-600"}`}>
                                {assetIsPositive ? <ArrowUpIcon className="h-3 w-3" /> : <ArrowDownIcon className="h-3 w-3" />}
                                {asset.lossGainPercentage.toFixed(2)}%
                              </span>
                            </div>
                  ...
                  <tfoot>
                    <tr className="border-t-2 font-semibold bg-muted/30">
                      ...
                      <td className="py-3 px-4 text-right text-sm">100%</td>
                      <td className="py-3 px-4 text-right text-sm">—</td>
                      <td className="py-3 px-4 text-right text-sm">${totalCost.toLocaleString()}</td>
                      <td className="py-3 px-4 text-right text-sm">—</td>
                      <td className="py-3 px-4 text-right text-sm">${totalValue.toLocaleString()}</td>
                      <td className="py-3 px-4 text-right">
                        <div className="flex flex-col items-end gap-0.5">
                          <span className={`text-sm font-bold ${isPositive ? "text-green-600" : "text-red-600"}`}>
                            ${Math.abs(totalLossGain).toLocaleString()}
          ...
          <Card>
            ...
            <CardContent>
              <ChartContainer
                config={{ lossGain: { label: "Gain/Loss", color: "#3b82f6" } }}
                className="h-[300px]"
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={assets}>
        ...
        <TabsContent value="allocation" className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <ChartContainer
                  config={{ value: { label: "Value", color: "#3b82f6" } }}
                  className="h-[350px]"
                >
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
              ...
              <div className="space-y-3">
                {assets.map((asset: any, index: number) => {
                  const percentage = totalValue > 0
                    ? ((asset.closeValue / totalValue) * 100).toFixed(2)
                    : "0.00"
